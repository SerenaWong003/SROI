...
# --- ส่วนอธิบายศัพท์ทางเทคนิค ---
with st.expander("ℹ️ คำอธิบายศัพท์เทคนิคในการคำนวณ SROI (Glossary)", expanded=False):
    st.markdown("""
    <div class="info-box">
    <b>1. Deadweight (ผลลัพธ์ส่วนเกิน):</b> มูลค่าของผลลัพธ์ที่เกิดขึ้นอยู่แล้วแม้จะไม่มีโครงการนี้เกิดขึ้นก็ตาม <br>
    <b>2. Displacement (การแทนที่):</b> ผลของโครงการที่ไปทำให้เกิดปัญหาในพื้นที่อื่น หรือเป็นการย้ายปัญหาจากจุดหนึ่งไปอีกจุดหนึ่ง <br>
    <b>3. Attribution (การรับรองสิทธิ์):</b> สัดส่วนของผลลัพธ์ที่เกิดจากหน่วยงานอื่นหรือปัจจัยภายนอกที่ไม่ใช่โครงการของเรา <br>
...
with st.sidebar:
    ...
    st.divider()
    st.caption("รายงาน: โปรดตรวจสอบงบประมาณให้ถูกต้องก่อนคำนวณ")
...
if 'calc_results' in st.session_state:
    ...
    with e_col1:
        # CSV Export (ใช้ utf-8-sig เพื่อให้ Excel อ่านไทยออก)
    ...
    with e_col2:
        ...
        def generate_pdf_file(data):
            ...
            for d in data['details']:
                # กรองอักขระเพื่อป้องกัน PDF Crash หากมีภาษาไทย
