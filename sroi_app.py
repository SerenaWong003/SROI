...
# --- 2. ปรับแต่ง CSS - บังคับสีดำบนพื้นหลังขาว และ Glossary สีดำ ---
st.markdown("""
    <style>
...
# --- 3. ฟังก์ชันสำหรับล้างข้อมูล (Reset System) ---
def reset_system():
...
with st.expander("ℹ️ คำอธิบายศัพท์เทคนิคในการคำนวณ SROI", expanded=False):
    ...
    <p><b>2. Displacement (การแทนที่):</b> การย้ายปัญหาจากจุดหนึ่งไปอีกจุดหนึ่ง หรือทำให้เกิดปัญหาที่จุดอื่นแทน</p>
...
# --- 5. Logic การคำนวณรายปี (TPV, NPV, SROI) ---
...
# --- 7. การจัดการรายการผู้มีส่วนได้เสีย (Dynamic Rows) ---
...
st.subheader("📝 รายละเอียดข้อมูลและปัจจัยปรับลด เพิ่มได้สูงสุด 10 รายการ ")
...
if 'res' in st.session_state:
    ...
    # ตารางรายปี (Present Value of each year)
    ...
    with e_col1:
        # 1. Export CSV (รองรับภาษาไทยใน Excel)
        csv_data = df_with_summary.to_csv(index=False).encode('utf-8-sig')
        st.download_button("Download Full CSV (Excel)", csv_data, f"SROI_{r['p_name']}.csv", "text/csv")
    
    with e_col2:
        # 2. Export PDF (รองรับภาษาไทยด้วยฟอนต์ที่ท่านเตรียมไว้)
        def generate_pdf(data):
            pdf = FPDF()
            pdf.add_page()
            # พยายามเรียกฟอนต์ THSarabun.ttf จาก GitHub
            font_file = "THSarabun.ttf"
            if os.path.exists(font_file):
                pdf.add_font("THSarabun", "", font_file)
                pdf.set_font("THSarabun", size=18)
            else:
                pdf.set_font("Helvetica", size=16) # Fallback ถ้าหาฟอนต์ไม่เจอ
                
            pdf.cell(200, 10, txt="SROI Financial Analysis Report", ln=True, align='C')
            pdf.ln(10)
            ...
            pdf.cell(200, 10, txt=f"Total PV (TPV): {data['tpv']:,.2f} บาท", ln=True)
            pdf.cell(200, 10, txt=f"Net PV (NPV): {data['npv']:,.2f} บาท", ln=True)
            pdf.cell(200, 10, txt=f"งบประมาณ (Total Input): {data['t_input']:,.2f} บาท", ln=True)
            pdf.ln(10)
            pdf.cell(200, 10, txt="สรุปรายรายการ:", ln=True)
            for d in data['details']:
                pdf.cell(200, 10, txt=f"- {d['ผู้มีส่วนได้เสีย/ผลลัพธ์']}: TPV = {d['Total PV (TPV)']:,.2f} บาท", ln=True)
            return pdf.output()
